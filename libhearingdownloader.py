...
def printWarranty():
    warrantyDisclaimer = [
        ...
        "WITHOUT WARRANTY OF ANY KIND, EXPLICIT OR IMPLICIT, INCLUDING BUT NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY,",
        ...
        "OF THE CHECKER. THEY HAVE NO REQUIREMENT TO PROVIDE ANY SUPPORT OR WARRANTY TO ANY SOFTWARE ASSOCIATED WITH THE USE",
    ...
    printDisclaimer(warrantyDisclaimer)
...
def downloadFile(url, saveLocation, downloadDescription):
    ...
    while streamRetries > 0:
        ...
        streamRetries -= 1
    if (streamRetries == 0):
        print("\n" + Fore.RED + "Error" + Style.RESET_ALL + ": Can not get proper file data stream from " + Fore.GREEN + url.replace(" ","%20") + Style.RESET_ALL + "\nYou can try to open this URI manually with web browsers. It might work.\n")
